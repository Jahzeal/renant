...
// Import the necessary types and hook
import { useAuth, ProfileUpdateDto } from "@/hooks/use-auth"; 
...
export default function ProfilePage() {
    ...
    // 💡 FIX: Use separate selectors for state (user) and actions (updateProfileData).
    // This prevents the infinite loop error (The result of getSnapshot should be cached...)
    const user = useAuth((state) => state.user);
    const updateProfileData = useAuth((state) => state.updateProfileData);

    // --- STATE INITIALIZATION ---
    // Note: If your API returns PascalCase (Firstname), you might need to adjust user?.firstName here too.
    ...
    // --- HANDLERS (Calling the centralized updateProfileData) ---
    ...
    //     // This remains local until schema is updated
    //     setDisplayScreenName(screenName);
    //     console.warn("ScreenName update requires schema modification.");
    // };
    ...
    const handleSavePhoto = (imageUrl: string) => {
        ...
        console.warn("Photo update is currently unconnected.");
    };
    ...
    return (
        <>
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div className="flex items-start justify-between py-4 border-b border-border/50 gap-4">
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <p className="text-sm text-foreground/60 mt-1">Your first and last given names. Updates are reflected across all Zillow experiences.</p>
                                        </div>
                                    ...
                                    <div className="flex items-start justify-between py-4 border-b border-border/50 gap-4">
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <p className="text-sm text-foreground/60 mt-1">Personalize your profile pic with a custom photo.</p>
                                        </div>
                                        <div className="flex flex-col items-end gap-2 flex-shrink-0">
                                    ...
                                    <div className="flex items-start justify-between py-4 gap-4">
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <p className="text-sm text-foreground/60 mt-1">Manage the reviews you've written for professionals, rentals, and more.</p>
                                        </div>
                                    ...
                                    <div className="flex items-start justify-between py-4 border-b border-border/50 gap-4">
                                        ...
                                        <div className="flex flex-col items-end gap-2 flex-shrink-0">
                                            <div className="text-right">
                                                ...
                                                <p className="text-xs text-foreground/50 mt-1">(unverified)</p>
                                            </div>
            ...
            {/* Modals are correctly connected to the new async handlers */}
            <EditNameModal
                isOpen={editNameOpen}
