...
interface AuthStore {
  ...
  signUp: (
    data: Omit<User, "id" | "profileImage"> & { password: string }
  ) => Promise<void>;
  signOut: () => void;
...
const normalizeProfileResponse = (
  ...
  if (apiData.profileImage !== undefined)
    update.profileImage = apiData.profileImage;
  ...
  persist(
    (set) => ({
      ...
      updateProfileData: async (data: ProfileUpdateDto) => {
        ...
        try {
          const res = await fetch(
            `${process.env.NEXT_PUBLIC_API_URL}/users/update-me`,
            {
              method: "PATCH",
              headers: {
          ...
          set((state) => {
            ...
            const normalizedUpdate = normalizeProfileResponse(
              apiResponse,
              state.user
            );
